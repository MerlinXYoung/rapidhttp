...
class Status {
  public:
    ...
    enum {
         ...
         */
        Continue = 100,  //!< Indicates that the initial part of a request has been received and has
                         //!< not yet been rejected by the server.
        SwitchingProtocols =
            101,  //!< Indicates that the server understands and is willing to comply with the
                  //!< client's request, via the Upgrade header field, for a change in the
                  //!< application protocol being used on this connection.
        Processing = 102,  //!< Is an interim response used to inform the client that the server has
                           //!< accepted the complete request, but has not yet completed it.
        EarlyHints =
            103,  //!< Indicates to the client that the server is likely to send a final
        ...
        Created = 201,   //!< Indicates that the request has been fulfilled and has resulted in one
                         //!< or more new resources being created.
        Accepted = 202,  //!< Indicates that the request has been accepted for processing, but the
                         //!< processing has not been completed.
        NonAuthoritativeInformation =
            203,  //!< Indicates that the request was successful but the enclosed payload has been
                  //!< modified from that of the origin server's 200 (OK) response by a transforming
                  //!< proxy.
        NoContent =
            204,  //!< Indicates that the server has successfully fulfilled the request and that
                  //!< there is no additional content to send in the response payload body.
        ResetContent =
            205,  //!< Indicates that the server has fulfilled the request and desires that
                  //!< the user agent reset the \"document view\", which caused the request
                  //!< to be sent, to its original state as received from the origin server.
        PartialContent = 206,  //!< Indicates that the server is successfully fulfilling a range
        ...
        AlreadyReported =
            208,  //!< Used inside a DAV:propstat response element to avoid enumerating the internal
                  //!< members of multiple bindings to the same collection repeatedly. [RFC 5842]
        IMUsed = 226,  //!< The server has fulfilled a GET request for the resource, and the
         ...
         */
        MultipleChoices = 300,  //!< Indicates that the target resource has more than one
                                //!< representation, each with its own more specific identifier, and
                                //!< information about the alternatives is being provided so that
                                //!< the user (or user agent) can select a preferred representation
                                //!< by redirecting its request to one or more of those identifiers.
        MovedPermanently =
            301,  //!< Indicates that the target resource has been assigned a new permanent URI and
        ...
        SeeOther = 303,  //!< Indicates that the server is redirecting the user agent to a different
        ...
        NotModified = 304,  //!< Indicates that a conditional GET request has been received and
                            //!< would have resulted in a 200 (OK) response if it were not for the
                            //!< fact that the condition has evaluated to false.
        UseProxy = 305,     //!< \deprecated \parblock Due to security concerns regarding in-band
                         //!< configuration of a proxy. \endparblock The requested resource MUST be
        ...
        PermanentRedirect = 308,  //!< The target resource has been assigned a new permanent URI and
         ...
         */
        BadRequest = 400,  //!< Indicates that the server cannot or will not process the request
                           //!< because the received syntax is invalid, nonsensical, or exceeds some
                           //!< limitation on what the server is willing to process.
        Unauthorized = 401,  //!< Indicates that the request has not been applied because it lacks
        ...
        NotFound = 404,  //!< Indicates that the origin server did not find a current representation
                         //!< for the target resource or is not willing to disclose that one exists.
        MethodNotAllowed =
            405,  //!< Indicates that the method specified in the request-line is known
        ...
        ProxyAuthenticationRequired =
            407,  //!< Is similar to 401 (Unauthorized), but indicates that the client needs to
                  //!< authenticate itself in order to use a proxy.
        RequestTimeout = 408,  //!< Indicates that the server did not receive a complete request
                               //!< message within the time that it was prepared to wait.
        Conflict = 409,  //!< Indicates that the request could not be completed due to a conflict
                         //!< with the current state of the resource.
        Gone = 410,  //!< Indicates that access to the target resource is no longer available at the
                     //!< origin server and that this condition is likely to be permanent.
        LengthRequired = 411,  //!< Indicates that the server refuses to accept the request without
        ...
        ContentTooLarge =
            413,  //!< Indicates that the server is refusing to process a request because the
        ...
        URITooLong =
            414,  //!< Indicates that the server is refusing to service the request because the
                  //!< request-target is longer than the server is willing to interpret.
        UnsupportedMediaType = 415,  //!< Indicates that the origin server is refusing to service
                                     //!< the request because the payload is in a format not
                                     //!< supported by the target resource for this method.
        RangeNotSatisfiable =
            416,  //!< Indicates that none of the ranges in the request's Range header field overlap
                  //!< the current extent of the selected resource or that the set of ranges
                  //!< requested has been rejected due to invalid ranges or an excessive request of
                  //!< small or overlapping ranges.
        ExpectationFailed =
            417,  //!< Indicates that the expectation given in the request's Expect header field
                  //!< could not be met by at least one of the inbound servers.
        ImATeapot = 418,  //!< Any attempt to brew coffee with a teapot should result in the error
                          //!< code 418 I'm a teapot.
        MisdirectedRequest =
            421,  //!< Indicates that the request was directed at a server that is unable or
                  //!< unwilling to produce an authoritative response for the target URI.
        UnprocessableContent =
            422,  //!< Means the server understands the content type of the request entity (hence a
        ...
        FailedDependency =
            424,  //!< Means that the method could not be performed on the resource because the
                  //!< requested action depended on another action and that action failed.
        TooEarly = 425,  //!< Indicates that the server is unwilling to risk processing a request
                         //!< that might be replayed.
        UpgradeRequired = 426,  //!< Indicates that the server refuses to perform the request using
        ...
        TooManyRequests = 429,  //!< Indicates that the user has sent too many requests in a given
                                //!< amount of time (\"rate limiting\").
        RequestHeaderFieldsTooLarge =
            431,  //!< Indicates that the server is unwilling to process the
                  //!< request because its header fields are too large.
        UnavailableForLegalReasons =
            451,  //!< This status code indicates that the server is denying
                  //!< access to the resource in response to a legal demand.

        /*####### 5xx - Server Error #######*/
        /* Indicates that the server is aware that it has erred
         * or is incapable of performing the requested method.
         */
        InternalServerError = 500,  //!< Indicates that the server encountered an unexpected
                                    //!< condition that prevented it from fulfilling the request.
        NotImplemented = 501,  //!< Indicates that the server does not support the functionality
                               //!< required to fulfill the request.
        BadGateway = 502,      //!< Indicates that the server, while acting as a gateway or proxy,
        ...
        GatewayTimeout = 504,  //!< Indicates that the server, while acting as a gateway or proxy,
                               //!< did not receive a timely response from an upstream server it
                               //!< needed to access in order to complete the request.
        HTTPVersionNotSupported =
            505,  //!< Indicates that the server does not support, or refuses to support, the
                  //!< protocol version that was used in the request message.
        VariantAlsoNegotiates =
            506,  //!< Indicates that the server has an internal configuration error: the chosen
        ...
        InsufficientStorage = 507,  //!< Means the method could not be performed on the resource
                                    //!< because the server is unable to store the representation
                                    //!< needed to successfully complete the request.
        LoopDetected =
            508,  //!< Indicates that the server terminated an operation because it encountered an
                  //!< infinite loop while processing a request with "Depth: infinity". [RFC 5842]
        NotExtended = 510,  //!< \deprecated \parblock Obsoleted as the experiment has ended and
                            //!< there is no evidence of widespread use. \endparblock The policy for
                            //!< accessing the resource has not been met in the request. [RFC 2774]
